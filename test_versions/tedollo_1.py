...
# Dollo condition
print('Generating constraint for Dollo')	
c = 0
while c < num_clones:
	m = 0
	while m < num_mutations:
...
print('#~~~~~ Total Persistent characters: {0}#'.format(k_dollo))
...
def print_solution():
	...
	if model.status == GRB.Status.OPTIMAL or model.status == GRB.Status.TIME_LIMIT:
		...
		# eh = 0
		# ehi = 'a'
		# while eh < (k_dollo+1)*num_mutations:
		# 	if eh % (k_dollo+1)*num_mutations == 0:
...
f_out = open('test_results/dollo.txt', 'a+')
if k_dollo == 0:
	f_out.close()
	f_out = open('test_results/perfect.txt', 'a+')
print_solution()
f_out.close()
